...
fn assert_context_run<T>(context_vec: &mut ContextVec<T>, result: &str) where T: Node + Serialize + Deserialize {
    let runner = NodeRunner { tokens: vec!(
        NodeToken::Get,
        NodeToken::ChainContext
    )};
    assert_eq!(result, context_vec.node_step(runner).as_str());
}

#[test]
fn test_chain_context() {
    let mut context_vec = test_vec();
    assert_context_run(&mut context_vec, "|");

    context_vec.set_context(0);
    assert_context_run(&mut context_vec, "|10|");

    context_vec.set_context(1);
    assert_context_run(&mut context_vec, "|1337|");

    context_vec.set_context(2);
    assert_context_run(&mut context_vec, "|42|");

    context_vec.set_context_vec(vec!(2, 0));
    assert_context_run(&mut context_vec, "|42|10|");

    context_vec.set_context_vec(vec!(0, 1, 2));
    assert_context_run(&mut context_vec, "|10|1337|42|");
}
...
fn test_push() {
    let mut context_vec = test_vec();
    assert_eq!(context_vec.get_vec().len(), 3);
    context_vec.push(99);
    assert_eq!(context_vec.get_vec().len(), 4);
    assert_eq!(context_vec.get_vec()[3], 99);
}
...
fn test_insert() {
    ...
    assert_eq!(context_vec.get_vec().len(), 4);
    assert_eq!(context_vec.get_vec()[0], 10);
    assert_eq!(context_vec.get_vec()[1], 1337);
    assert_eq!(context_vec.get_vec()[2], 42);
    assert_eq!(context_vec.get_vec()[3], 99);
    ...
    assert_eq!(context_vec.get_vec().len(), 5);
    assert_eq!(context_vec.get_vec()[0], 10);
    assert_eq!(context_vec.get_vec()[1], 101);
    assert_eq!(context_vec.get_vec()[2], 1337);
    assert_eq!(context_vec.get_vec()[3], 42);
    assert_eq!(context_vec.get_vec()[4], 99);
}
...
fn test_pop() {
    ...
    assert_eq!(context_vec.get_context().len(), 1);
    assert_eq!(context_vec.get_vec().len(), 2);
}
